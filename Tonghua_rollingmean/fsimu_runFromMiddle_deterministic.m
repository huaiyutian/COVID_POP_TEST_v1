...
function ydot = fsimu(T,theta,xdata,parpop,par_tau,Inital_Middle)
...
HS(1) = Inital_Middle(1); %susceptible
HLa(1) = Inital_Middle(2); %latent asymptomatic 
HIa(1) = Inital_Middle(3); % infectious asymptomatic
HLs(1) = Inital_Middle(4); %latent symptomatic 
HLp(1) = Inital_Middle(5); %presymptomatic
HI(1) = Inital_Middle(6); %infectious symptomatic
HR(1) = Inital_Middle(7); %removed infectious indivial


HN(1)= Ptonghua;%all population
HT(1) = 0; %the identified cases through the population-level-testing
HC(1) = 0; %the identified cases through the contact tracing.
DHC(1)=0;
DHT(1)=0;
