...
function ydot = fsimu(T,theta,xdata,parpop,par_tau,HS0,HR0,p)
...
HS(1) = HS0; %susceptible
HR(1) = HR0; %removed infectious indivial
...
HS(i)=HS(i-1)-tmp_Exporue+w*HR(i-1);
...
% removed population
HR(i)=HR(i-1)+(1-Tau(i-1))*(1-Theta)*Mu.*HI(i-1)+(1-Tau(i-1))*(1-Theta)*Mu.*HIa(i-1)-w*HR(i-1);
